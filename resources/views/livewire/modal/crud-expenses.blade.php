...
<fieldset class="form-group">
    <select  wire:model="expenses_id" class="form-control" >
        <option>.......</option>
        @forelse ($workers as $worker)
        <option  value="{{$worker->team_id}}">{{$worker->user->name}}</option>
@empty
@endforelse
</select>
</fieldset>  
@error('expenses_id') <div class="alert alert-danger" role="alert">
    <p class="mb-0">{{ $message }} </p>
</div>   
@enderror 
